...
end_going_arg (void)
{
    if (arg_going)
    {
        ...
        //LOGV("string:%s", string);
        //if (0 == memcmp(string, "cc1", 3)) //2017-10-28,zhou.weiguo, hardcode for ./cc1
        //   store_arg ("./cc1", delete_this_arg, this_is_output_file);
...
do_spec_1 (const char *spec, int inswitch, const char *soft_matched_part)
{
    ...
    //LOGV("spec:%s, soft_matched_part:%s", spec, soft_matched_part);
    ...
    while ((c = *p++))
    {
        ...
        case '%':
                   ...
                   and don't execute any more commands for this file.  */
            {
                ...
                buf[p - q] = 0;
                error ("%s", _(buf));
                LOGV("return -1, leave %s", __FUNCTION__);
                return -1;
            }
            break;
            case 'n':
...
driver::do_spec_on_infiles () const
{
    ...
    for (i = 0; (int) i < n_infiles; i++)
    {
        ...
        LOGV("gcc_input_filename:%s", gcc_input_filename);
